...
public class Hoe extends HoeItem
{
	...
	public Hoe(ModdedTier tier, int durabilityMultiplier)
	{
	...
	public Hoe build(float reach, float movespeed)
	{
		...
		builder.put(Attributes.ATTACK_DAMAGE, new AttributeModifier(ATTACK_DAMAGE_MODIFIER, "Hoe modifier", (double) getAttackDamage(), Operation.ADDITION));
	...
	public Hoe rebuild()
	{
		...
		if (movespeed != 0)
		{
			builder.put(Attributes.MOVEMENT_SPEED, new AttributeModifier(MOVESPEED, "Hoe modifier", (double) movespeed, Operation.MULTIPLY_TOTAL));
		}
		if (reach != 0)
		{
			builder.put(ForgeMod.REACH_DISTANCE.get(), new AttributeModifier(REACH, "Hoe modifier", (double) reach, Operation.MULTIPLY_TOTAL));
		}
		modifiers = builder.build();
	...
	public Hoe rebuildWith(Attribute attribute, UUID id, String modifiername, double value, Operation valuetype)
	{
		Builder<Attribute, AttributeModifier> builder = ImmutableMultimap.builder();
		builder.putAll(modifiers);
